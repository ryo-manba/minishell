...
/*   Updated: 2021/09/30 22:42:53 by yokawada         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ms_analyzer.h"

static int	ex_fx_match_mark_step(t_ex_fx_dpcursor *dc, char pat[2], int qat[2])
{
	...
	qat[1] = dc->pattern_head->token_id != XI_DQUOTED
		&& dc->pattern_head->token_id != XI_SQUOTED;
	return (1);
}

static void	ex_fx_exec_match2_inner(t_ex_fx_dpcursor *dc, t_ex_token *file,
	size_t j)
{
	size_t	i;
	char	pat[2];
...
static int	ex_fx_exec_match2(t_ex_fx_dpcursor *dc, t_ex_token *file)
{
	size_t	j;
	...
	while (j <= file->n)
	{
		ex_fx_exec_match2_inner(dc, file, j);
		j += 1;
		ft_bzero(dc->dp[j % 2], (dc->n + 1) * sizeof(unsigned char));
	}
...
static void	ex_fx_match_loop2(t_ex_state *state, t_ex_unit_cursor *cursor,
