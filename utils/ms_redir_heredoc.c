...
/*   Updated: 2021/09/10 19:36:49 by rmatsuka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ms_redirect_heredoc(t_redir *redir, t_shellvar *var)
{
	...
	if (ms_heredoc_read(&lst, redir->operand_right->token) == MS_EXEC_FAIL) // 標準入力から読み取る
		return (MS_EXEC_FAIL);
	...
	if (close(pipefd[0]) == -1)
		return (MS_EXEC_FAIL);
	ms_heredoc_write(lst, var, quoted, pipefd[1]); // 展開したやつを改行区切りでパイプに書き込む
	return (MS_EXEC_SUCC);
}
